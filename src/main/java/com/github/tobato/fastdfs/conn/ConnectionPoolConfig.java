...
public class ConnectionPoolConfig extends GenericKeyedObjectPoolConfig {
    ...
    public ConnectionPoolConfig(int fdfsMaxTotal, boolean fdfsTestWhileIdle, boolean fdfsBlockWhenExhausted, 
    		long fdfsMaxWaitMillis, long fdfsMinEvictableIdletimeMillis, long fdfsTimeBetweenEvictionRunsMillis, 
    		int fdfsMumTestsPerevictionRun){
        // 从池中借出的对象的最大数目
        setMaxTotal(fdfsMaxTotal);
        // 在空闲时检查有效性
        setTestWhileIdle(fdfsTestWhileIdle);
        // 连接耗尽时是否阻塞(默认true)
        setBlockWhenExhausted(fdfsBlockWhenExhausted);
        // 获取连接时的最大等待毫秒数100
        setMaxWaitMillis(fdfsMaxWaitMillis);
        // 视休眠时间超过了180秒的对象为过期
        setMinEvictableIdleTimeMillis(fdfsMinEvictableIdletimeMillis);
        // 每过60秒进行一次后台对象清理的行动
        setTimeBetweenEvictionRunsMillis(fdfsTimeBetweenEvictionRunsMillis);
        // 清理时候检查所有线程
        setNumTestsPerEvictionRun(fdfsMumTestsPerevictionRun);
        // 配置jmx
        this.setJmxNameBase(FDFS_JMX_NAME_BASE);
        this.setJmxNamePrefix(FDFS_JMX_NAME_PREFIX);
    }
